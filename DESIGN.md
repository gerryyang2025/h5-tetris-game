...
- 十字方向键尺寸：160px × 160px，中心圆 45px，按钮 50px × 50px
- 功能按钮尺寸：50px × 50px
...
- 排行榜功能：游戏结束后自动保存成绩
- 幽灵方块：通过 `getGhostPosition()` 计算下落终点位置，渲染时使用 `ghost` 类显示半透明轮廓

### 4. 幽灵方块系统
- CSS 样式：`.cell.ghost { opacity: 0.3; }`
- 位置计算：`getGhostPosition(piece)` 逐行向下检测碰撞，返回最终 Y 坐标
- 渲染逻辑：先渲染幽灵方块（半透明），再渲染实际方块（实心），覆盖幽灵位置
...
- 旋转使用矩阵转置算法
- 碰撞检测：边界检查 + 棋盘占用检查
- 幽灵方块：通过 `getGhostPosition()` 计算方块落点位置，渲染时先绘制幽灵再绘制实心方块
